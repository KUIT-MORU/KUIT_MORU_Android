...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun signUp(email: String, password: String): Result<Unit> {
        ...
        Log.d(
            "signup",
            "[$traceId] call: /api/auth/signup, email=$email, pwdLen=${password.length}"
        )
        ...
        return try {
            ...
            Log.d(
                "signup",
                "[$traceId] ok in ${ms}ms, tokens(access=${resp.accessToken.take(6)}...${
                    resp.accessToken.takeLast(4)
                }, refresh=${resp.refreshToken.take(6)}...${resp.refreshToken.takeLast(4)})"
            )
            ...
            Log.e(
                "signup",
                "[$traceId] http ${e.code()} in ${ms}ms: ${e.message()}",
                e
            )
            Result.failure(e)
